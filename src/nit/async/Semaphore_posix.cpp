...
#include "nit/async/Semaphore_posix.h"
...
SemaphoreImpl::SemaphoreImpl(int n, int max): _n(n), _max(max)
{
	...
	if (pthread_mutex_init(&_mutex, NULL))
		NIT_THROW_FMT(EX_SYSTEM, "cannot create semaphore (mutex)");
	if (pthread_cond_init(&_cond, NULL))
		NIT_THROW_FMT(EX_SYSTEM, "cannot create semaphore (condition)");
}
...
void SemaphoreImpl::waitImpl()
{
	if (pthread_mutex_lock(&_mutex))
		NIT_THROW_FMT(EX_SYSTEM, "wait for semaphore failed (lock)"); 
	while (_n < 1) 
	{
		if (pthread_cond_wait(&_cond, &_mutex))
		{
			pthread_mutex_unlock(&_mutex);
			NIT_THROW_FMT(EX_SYSTEM, "wait for semaphore failed");
		}
	}
	--_n;
	pthread_mutex_unlock(&_mutex);
...
bool SemaphoreImpl::waitImpl(long milliseconds)
{
	...
	while (_n < 1) 
	{
		if ((rc = pthread_cond_timedwait(&_cond, &_mutex, &abstime)))
		{
			...
			pthread_mutex_unlock(&_mutex);
			NIT_THROW_FMT(EX_SYSTEM, "cannot wait for semaphore");
		}
	}
	if (rc == 0) --_n;
	pthread_mutex_unlock(&_mutex);
