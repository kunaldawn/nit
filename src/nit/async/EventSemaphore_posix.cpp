...
EventSemaphoreImpl::EventSemaphoreImpl(bool autoReset): _auto(autoReset), _state(false)
{
	if (pthread_mutex_init(&_mutex, NULL))
		NIT_THROW_FMT(EX_SYSTEM, "cannot create event (mutex)");
	if (pthread_cond_init(&_cond, NULL))
		NIT_THROW_FMT(EX_SYSTEM, "cannot create event (condition)");
}
...
void EventSemaphoreImpl::waitImpl()
{
	...
	while (!_state) 
	{
		if (pthread_cond_wait(&_cond, &_mutex))
		{
			...
			NIT_THROW_FMT(EX_SYSTEM, "wait for event failed");
		}
	}
	if (_auto)
		_state = false;
