...
#include "nit/async/Mutex_posix.h"
...
MutexImpl::MutexImpl()
{
	...
	if (pthread_mutex_init(&_mutex, &attr))
	{
		...
		NIT_THROW_FMT(EX_SYSTEM, "cannot create mutex");
	}
	pthread_mutexattr_destroy(&attr);
}
...
{
#if defined(NIT_HAVE_MUTEX_TIMEOUT)
	...
	if (rc == 0)
		...
		NIT_THROW(EX_SYSTEM, "cannot lock mutex");
#else
	const int sleepMillis = 5;
	PocoTimestamp now;
	PocoTimestamp::TimeDiff diff(PocoTimestamp::TimeDiff(milliseconds)*1000);
	do
	{
