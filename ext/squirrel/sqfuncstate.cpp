...
{
	switch(sqi_type(o)){
		case OT_STRING:	scprintf(_SC("\"%s\""),sqi_stringval(o));break;
		case OT_FLOAT: scprintf(_SC("{%f}"),sqi_float(o));break;
		case OT_INTEGER: scprintf(_SC("{%d}"),sqi_integer(o));break;
		case OT_BOOL: scprintf(_SC("%s"),sqi_integer(o)?_SC("true"):_SC("false"));break;
		default: scprintf(_SC("(%s %p)"),GetTypeName(o),(void*)sqi_rawval(o));break; break; //shut up compiler
	}
}
...
void SQFuncState::Dump(SQFunctionProto *func)
{
	...
	scprintf(_SC("--------------------------------------------------------------------\n"));
	scprintf(_SC("*****FUNCTION [%s]\n"),sqi_type(func->_name)==OT_STRING?sqi_stringval(func->_name):_SC("unknown"));
	...
	while((idx=sqi_table(_literals)->Next(false,refidx,key,val))!=-1) {
		refidx=idx;
		templiterals[sqi_integer(val)]=key;
	}
	...
	for(i=0;i<_instructions.size();i++){
		...
		if(inst.op==_OP_LOAD || inst.op==_OP_DLOAD || inst.op==_OP_PREPCALLK || inst.op==_OP_GETK ){
			...
			if(lidx >= 0xFFFFFFFF)
				...
				while(((refidx=sqi_table(_literals)->Next(false,refo,key,val))!= -1) && (sqi_integer(val) != lidx)) {
					refo = refidx;	
				}
				DumpLiteral(key);
			}
...
{
	SQObjectPtr val;
	if(!sqi_table(_literals)->Get(cons,val))
	{
		val = _nliterals;
		sqi_table(_literals)->NewSlot(cons,val);
		_nliterals++;
...
{
	SQObjectPtr val;
	if(sqi_table(_sharedstate->_consts)->Get(name,val)) {
		e = val;
		return true;
	}
	return false;
...
bool SQFuncState::IsLocal(SQUnsignedInteger stkpos)
{
	...
	else if(sqi_type(_vlocals[stkpos]._name)!=OT_NULL)return true;
	return false;
}
...
SQInteger SQFuncState::GetLocalVariable(const SQObject &name)
{
	...
	while(locals>=1){
		...
		if(sqi_type(lvi._name)==OT_STRING && sqi_string(lvi._name)==sqi_string(name)){
			return locals-1;
		}
		locals--;
	}
...
SQInteger SQFuncState::GetOuterVariable(const SQObject &name)
{
	...
	for(SQInteger i = 0; i<outers; i++) {
		if(sqi_string(_outervalues[i]._name) == sqi_string(name))
			return i;
	}
	SQInteger pos=-1;
	if(_parent) { 
...
SQObject SQFuncState::CreateString(const SQChar *s,SQInteger len)
{
	...
	sqi_table(_strings)->NewSlot(ns,(SQInteger)1);
	return ns;
}
...
SQObject SQFuncState::CreateTable()
{
	...
	sqi_table(_strings)->NewSlot(nt,(SQInteger)1);
	return nt;
}
