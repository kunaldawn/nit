...
void SQLexer::Init(SQSharedState *ss, SQLEXREADFUNC rg, SQUserPointer up,CompilerErrorFunc efunc,void *ed)
{
	...
	ADD_KEYWORD(is,TK_IS);
	ADD_KEYWORD(mod,TK_INTMOD);
	ADD_KEYWORD(null, TK_NULL);
	ADD_KEYWORD(property,TK_PROPERTY);
	ADD_KEYWORD(return, TK_RETURN);
	ADD_KEYWORD(resume, TK_RESUME);
	ADD_KEYWORD(require,TK_REQUIRE);		
...
const SQChar *SQLexer::Tok2Str(SQInteger tok)
{
	...
	while((nitr = _keywords->Next(false,itr, key, val)) != -1) {
		itr = (SQInteger)nitr;
		if(((SQInteger)sqi_integer(val)) == tok)
			return sqi_stringval(key);
	}
	return NULL;
}
...
SQInteger SQLexer::Lex()
{
	...
	while(CUR_CHAR != SQUIRREL_EOB) {
		...
		case _SC('='):
			NEXT();
			if (CUR_CHAR == _SC('>')) { NEXT(); RETURN_TOKEN(TK_LAMBDA); }
		...
		case _SC('@'): {
			...
			// verbatim string : interprets `@"hello " world"@` -> as `hello " world`
...
SQInteger SQLexer::GetIDType(SQChar *s)
{
	...
	if(_keywords->Get(SQString::Create(_sharedstate, s), t)) {
		return SQInteger(sqi_integer(t));
	}
			...
			case _SC('\\'):
				if(verbatim) {
					...
					case _SC('u'): NEXT(); {
						...
						temp[n] = 0;
						SQChar *sTemp;
						int unichar = scstrtoul(temp,&sTemp,16);

						char utf8Seq[4];
						size_t utf8Len = nit::Unicode::toUtf8Char(unichar, utf8Seq);
						for (size_t i=0; i<utf8Len; ++i)
							APPEND_CHAR((SQChar)utf8Seq[i]);
					}
				    break;
