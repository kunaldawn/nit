...
#define SQSTD_BLOB_TYPE_TAG (&typeid(SQSTDBlob))
...
{
	if (sq_gettype(v, 2) != OT_INTEGER && sq_gettype(v, 2) != OT_FLOAT)
	{
		sq_reseterror(v); // SQ_ERROR with reseterror means 'not found'.. provokes further fallback matching
		return SQ_ERROR;
	}
	SETUP_BLOB(v);
	SQInteger idx;
...
static SQInteger _blob_releasehook(HSQUIRRELVM v, SQUserPointer p, SQInteger size)
{
...
#define _DECL_BLOB_FUNC(name,nparams,typecheck,help) {_SC(#name),_blob_##name,nparams,typecheck,NULL,_SC(help)}
static SQRegFunction _blob_methods[] = {
	_DECL_BLOB_FUNC(constructor,-1,_SC("xn"),	"(size: int)"),
	_DECL_BLOB_FUNC(resize,2,_SC("xn"),			"(size: int)"),
	_DECL_BLOB_FUNC(swap2,1,_SC("x"),			"()"),
	_DECL_BLOB_FUNC(swap4,1,_SC("x"),			"()"),
	_DECL_BLOB_FUNC(_set,3,_SC("xnn"),			NULL),
	_DECL_BLOB_FUNC(_get,2,_SC("x."),			NULL),
	_DECL_BLOB_FUNC(_typeof,1,_SC("x"),			NULL),
	_DECL_BLOB_FUNC(_nexti,2,_SC("x"),			NULL),
	_DECL_BLOB_FUNC(_cloned,2,_SC("xx"),		NULL),
	{0,0,0,0}
};
...
#define _DECL_GLOBALBLOB_FUNC(name,nparams,typecheck,help) {_SC(#name),_g_blob_##name,nparams,typecheck,NULL,help}
static SQRegFunction bloblib_funcs[]={
	_DECL_GLOBALBLOB_FUNC(casti2f,2,_SC(".n"),	"(n: int): float"),
	_DECL_GLOBALBLOB_FUNC(castf2i,2,_SC(".n"),	"(f: float): int"),
	_DECL_GLOBALBLOB_FUNC(swap2,2,_SC(".n"),	"(n: int16): int16"),
	_DECL_GLOBALBLOB_FUNC(swap4,2,_SC(".n"),	"(n: int32): int32"),
	_DECL_GLOBALBLOB_FUNC(swapfloat,2,_SC(".n"),"(f: float): float"),
	{0,0}
};
