...
public:
	...
	void Error(const SQChar *s, ...)
	{
		...
		va_list vl;
		va_start(vl, s);
		SQInteger len = scvsnprintf(temp, sizeof(temp), s, vl);
		assert(len < 256);
		va_end(vl);
		compilererror = temp;
		longjmp(_errorjmp,1);
	...
	SQObject Expect(SQInteger tok, bool treatKeywordAsIdentifier = false)
	{
		if (treatKeywordAsIdentifier && _token >= TK_KEYWORD_START)
		{
			_token = TK_IDENTIFIER;
			_lex._svalue = &_lex._longstr[0];
		}
	...
	bool Compile(SQObjectPtr &o)
	{
		...
		if(setjmp(_errorjmp) == 0) {
			...
			if(_raiseerror && _ss(_vm)->_compilererrorhandler) {
				_ss(_vm)->_compilererrorhandler(_vm, compilererror, sqi_type(_sourcename) == OT_STRING?sqi_stringval(_sourcename):_SC("unknown"),
	...
	void Statement(bool closeframe = true)
	{
		...
		case TK_VAR:	LocalDeclStatement();	break;
		case TK_RETURN:
		case TK_YIELD: {
			SQOpcode op;
			if(_token == TK_RETURN) {
		...
		case TK_BREAK:
			...
			if(_fs->_within_count > 0) Error(_SC("can't 'break' in a with expr"));
		...
		case TK_CONTINUE:
			...
			if(_fs->_within_count > 0) Error(_SC("can't 'continue' in a with expr"));
		...
		case TK_CONST:
			{
			Error(_SC("deprecated 'const' keyword"));
			Lex();
			SQObject id = Expect(TK_IDENTIFIER);
			Expect('=');
			SQObject val = ExpectScalar();
			OptionalSemicolon();
			SQTable *enums = sqi_table(_ss(_vm)->_consts);
	...
	void Expression()
	{
		...
		case TK_MODEQ:{
			...
			if(_token != _SC('=') && ds == EXPR) Error(_SC("invalid assignment"));
				...
				case EXPR:
					{
						SQInteger src = _fs->PopTarget();
						SQInteger dst = _fs->TopTarget();
						_fs->AddInstruction(_OP_ASSIGN, dst, src);
					}
					break;
				case OBJECT:
				case BASE:
	...
	SQOpcode ChooseArithOpByToken(SQInteger tok)
	{
		switch(tok) {
			...
			case TK_INTDIV: return _OP_INTDIV;
			case TK_INTMOD: return _OP_INTMOD;
			default: assert(0);
		}
		return _OP_ADD;
	}
	...
	void MultExp()
	{
		PrefixedExpr();
		for(;;) switch(_token) {
		case _SC('*'): case _SC('/'): case _SC('%'): case TK_INTDIV: case TK_INTMOD:
	...
	void PrefixedExpr()
	{
		...
		for(;;) {
			switch(_token) {
 			case TK_IN: BIN_EXP(_OP_EXISTS, &SQCompiler::PrefixedExpr); break;
 			case TK_IS: BIN_EXP(_OP_INSTANCEOF, &SQCompiler::PrefixedExpr); break;
			...
			case _SC('.'):
				...
				if(_es.etype==BASE) {
					...
					{
						Lex();
						_fs->AddInstruction(_OP_LOAD, _fs->PushTarget(), _fs->GetConstant(_fs->_sharedstate->_constructoridx));
					}
					else
					{
						_fs->AddInstruction(_OP_LOAD, _fs->PushTarget(), _fs->GetConstant(Expect(TK_IDENTIFIER, true)));
					}
					Emit2ArgsOP(_OP_GET);
					pos = _fs->TopTarget();
					_es.etype = EXPR;
	...
	SQInteger Factor()
	{
		...
		case TK_THIS:{
				...
				if((pos = _fs->GetLocalVariable(id)) != -1) {
					...
					SQObject    constid;
					if(sqi_type(constant) == OT_TABLE) {
						Expect('.');
						constid = Expect(TK_IDENTIFIER);
						if(!sqi_table(constant)->Get(constid, constval)) {
							constval.Null();
							Error(_SC("invalid constant [%s.%s]"), sqi_stringval(id), sqi_stringval(constid));
						}
					}
					else {
						constval = constant;
					...
					SQObjectType ctype = sqi_type(constval);
					if(ctype == OT_INTEGER && (sqi_integer(constval) & (~0x7FFFFFFF)) == 0) {
						_fs->AddInstruction(_OP_LOADINT,   _es.epos, sqi_integer(constval));
					}
					else if(ctype == OT_FLOAT && sizeof(SQFloat)==sizeof(SQInt32) ) {
						SQFloat f = sqi_float(constval);
		...
		case _SC('$'):
			{
				...
				if (*utf8 != 0)
				{
					charValue = nit::Unicode::utf8Advance(utf8);
					if (*utf8 != 0)
		...
		case _SC('{'):
			{
				SQInteger table = _fs->PushTarget();
				_fs->AddInstruction(_OP_NEWOBJ, table, 0, NOT_TABLE);
				Lex();ParseTable();
				if (_token == ':') // delegate shortcut
				{
					Lex();
					_fs->AddInstruction(_OP_LOAD, _fs->PushTarget(), _fs->GetConstant(_fs->CreateString(_SC("setdelegate"))));
					SQInteger key = _fs->PopTarget();
					_fs->PopTarget(); // because OP_LOAD is to local
	...
	void ParseTable(SQInteger terminator = '}')
	{
		...
		while (_token != terminator)
		{
			...
			case TK_FUNCTION:
				{
					SQInteger tk = _token;
					Lex();
					SQObject id = Expect(TK_IDENTIFIER, true);
					Expect(_SC('('));
					_fs->AddInstruction(_OP_LOAD, _fs->PushTarget(), _fs->GetConstant(id));
					CreateFunction(id);
			...
			case TK_STRING_LITERAL: // JSON
				...
				break;
			default:
				_fs->AddInstruction(_OP_LOAD, _fs->PushTarget(), _fs->GetConstant(TypedID(NULL, true)));
			...
			_fs->AddInstruction(_OP_NEWSLOTA, 0, table, key, val);
		}
		_fs->SetIntructionParam(tpos, 1, nkeys);
		Lex();
	}

	// TODO: use this type tag
	void AddTagStr(SQObject* tag, SQChar delim, const char* st, int stlen)
	{
		if (tag == NULL) return;

		if (sqi_type(*tag) == OT_NULL)
			*tag = _fs->CreateString("");

		SQInteger taglen = sqi_string(*tag)->_len;
		SQChar* s = _ss(_vm)->GetScratchPad(rsl(taglen + stlen + 2));
		if (delim)
		{
			memcpy(s, sqi_stringval(*tag), rsl(taglen));
			s[taglen] = delim;
			memcpy(s + taglen + 1, st, rsl(stlen));
			*tag = _fs->CreateString(s, taglen + stlen + 1);
		}
		else
		{
			memcpy(s, sqi_stringval(*tag), rsl(taglen));
			memcpy(s + taglen, st, rsl(stlen));
			*tag = _fs->CreateString(s, taglen + stlen);
		}
	}

	void TypeTag(SQObject* tag, SQChar delim = 0)
	{
		if (_token == _SC(':')) Lex();

		SQObject id = Expect(TK_IDENTIFIER);
		AddTagStr(tag, delim, sqi_string(id)->_val, sqi_string(id)->_len);

		while (_token == _SC('.'))
		{
			Lex();
			id = Expect(TK_IDENTIFIER);
			AddTagStr(tag, _SC('.'), sqi_string(id)->_val, sqi_string(id)->_len);
		}
	...
	SQObject TypedID(SQObject* tag = NULL, bool treatKeywordAsIdentifier = false)
	{
		SQObject id = Expect(TK_IDENTIFIER, treatKeywordAsIdentifier);
	...
	void ParseClass()
	{
		// Prepare default initializer function
		SQFuncState* initfunc = _fs->PushChildState(_ss(_vm));
		initfunc->_name = _fs->_sharedstate->_initializeridx;
		initfunc->AddParameter(_fs->_sharedstate->_thisidx);
		initfunc->_sourcename = _sourcename;

		float propOrder = 1000.0f;
		...
		while(_token != _SC('}')) 
		{
			...
			case TK_DESTRUCTOR:
				{
					...
					Lex();
					SQObject id;
					if (tk == TK_CONSTRUCTOR)
						id = _fs->_sharedstate->_constructoridx;
					else if (tk == TK_DESTRUCTOR)
						id = _fs->_sharedstate->_destructoridx;
					else
						id = Expect(TK_IDENTIFIER, true);
					Expect(_SC('('));
					_fs->AddInstruction(_OP_LOAD, _fs->PushTarget(), _fs->GetConstant(id));
					CreateFunction(id);
					_fs->AddInstruction(_OP_CLOSURE, _fs->PushTarget(), _fs->_functions.size() - 1, 0);
			...
			case TK_PROPERTY:
				{
					if (isstatic) Error(_SC("'property' can't be static"));

					Lex();

					if (_token == _SC('#'))
					{
						if (!hasattrs) Error("'#' needs attributes");
						Lex();
						if (_token == TK_INTEGER)
							propOrder = (float) _lex._nvalue;
						else if (_token == TK_FLOAT)
							propOrder = _lex._fvalue;
						else
							Error("invalid property #");
						Lex();
					}

					SQObject id = TypedID(&typetag, true);
					_fs->AddInstruction(_OP_LOAD, _fs->PushTarget(), _fs->GetConstant(id));

					bool hasGetter = PropertyGetSet(id, GETSET_GET);
					if (!hasGetter) _fs->AddInstruction(_OP_LOADNULLS, _fs->PushTarget(), 1);
					bool hasSetter = PropertyGetSet(id, GETSET_SET);
					if (!hasSetter) _fs->AddInstruction(_OP_LOADNULLS, _fs->PushTarget(), 1);

					if (!hasGetter && !hasSetter) Error("expected 'get' or 'set'");
					isproperty = true;
				}
				break;
			case TK_CLASS:
				{
					...
					ClassExp();
					// EMIT: static _classname = <classname>
					SQInteger classObj = _fs->TopTarget();
					_fs->AddInstruction(_OP_LOAD, _fs->PushTarget(), _fs->GetConstant(_fs->_sharedstate->_classnameidx));
					SQInteger key = _fs->PopTarget();
					_fs->AddInstruction(_OP_NEWSLOTA, NEW_SLOT_STATIC_FLAG, classObj, key, className);
				}
				break;
			case TK_VAR:
				{
					if (isstatic) Error(_SC("'var' can't be static"));

					Lex();
					SQObject id = TypedID(NULL, true);

					// mark member field as null
					_fs->AddInstruction(_OP_LOAD, _fs->PushTarget(), _fs->GetConstant(id));
					_fs->AddInstruction(_OP_LOADNULLS, _fs->PushTarget(), 1);
			...
			if(_token == _SC(';')) Lex(); //optional semicolon

			if (isproperty)
			{
				if (hasattrs)
				{
					SQInteger attrs = _fs->TopTarget()-3;

					if (sqi_type(typetag) == OT_STRING)
					{
						_fs->AddInstruction(_OP_LOAD, _fs->PushTarget(), _fs->GetConstant(_fs->CreateString("type")));
						_fs->AddInstruction(_OP_LOAD, _fs->PushTarget(), _fs->GetConstant(typetag));
						SQInteger tag = _fs->PopTarget();
						_fs->AddInstruction(_OP_NEWSLOTA, 0, attrs, _fs->PopTarget(), tag);
					}

					// create 'order' attr
					_fs->AddInstruction(_OP_LOAD, _fs->PushTarget(), _fs->GetConstant(_fs->CreateString("order", -1)));
					_fs->AddInstruction(_OP_LOADFLOAT, _fs->PushTarget(), *((SQInt32 *)&propOrder));
					SQInteger order = _fs->PopTarget();
					_fs->AddInstruction(_OP_NEWSLOTA, 0, attrs, _fs->PopTarget(), order);

					propOrder += 1.0f;
				}

				SQInteger setter = _fs->PopTarget();
				SQInteger getter = _fs->PopTarget();
				SQInteger name = _fs->PopTarget();
				unsigned char flags = 0;

				if (hasattrs)
				{
					SQInteger attrs = _fs->PopTarget();
					assert(attrs == name-1);
					flags = NEW_SLOT_ATTRIBUTES_FLAG;
				}

				SQInteger table = _fs->TopTarget();
				_fs->AddInstruction(_OP_NEWPROP, flags, table, name, getter);
			}
			else
			{
				SQInteger val = _fs->PopTarget();
		...
		if (initfunc->_instructions.size() > 0)
		{
			// one or more init code - remember actual initializer function
			initfunc->AddLineInfos(_lex._prevtoken == _SC('\n')?_lex._lasttokenline:_lex._currentline, _lineinfo, true);
	...
	enum GetSetKeyword { GETSET_GET = 0x01, GETSET_SET = 0x02 };

	bool PropertyGetSet(SQObject& id, int allowed)
	{
		if (_token == TK_IDENTIFIER)
		{
			if (strcmp(_lex._svalue, "get") == 0 && allowed & GETSET_GET)
			{
				Lex();
				if (_token != _SC('{'))
			...
			if (strcmp(_lex._svalue, "set") == 0 && allowed & GETSET_SET)
			{
				Lex();
	...
	// TODO: implement a conditional function elimination mechanism
// 	void AssertStatement()
// 	{
// 		if (_ss(_vm)->_enableasserts)
// 		{
// 			CommaExpr();
// 			_fs->PopTarget();
// 		}
...
// 	}
	void ForStatement()
	{
		...
		BEGIN_SCOPE();
		Expect(_SC('('));
		if(_token == TK_VAR) LocalDeclStatement();
		else if(_token != _SC(';')){
			CommaExpr();
			_fs->PopTarget();
		}
	...
	void ForEachStatement()
	{
		...
		Lex(); Expect(_SC('(')); valname = TypedID();
		if(_token == _SC(',')) {
			idxname = valname;
			Lex(); valname = TypedID();
		}
	...
	void FunctionStatement()
	{
		...
		_fs->PushTarget(0);
		_fs->AddInstruction(_OP_LOAD, _fs->PushTarget(), _fs->GetConstant(id));
		// TODO: TK_DOUBLE_COLON HERE OBSOLETED
		if(_token == _SC('.') || _token == TK_DOUBLE_COLON) Emit2ArgsOP(_OP_GET);
		
		while(_token == _SC('.') || _token == TK_DOUBLE_COLON) {
			Lex();
			id = Expect(TK_IDENTIFIER);
			_fs->AddInstruction(_OP_LOAD, _fs->PushTarget(), _fs->GetConstant(id));
			if(_token == _SC('.') || _token == TK_DOUBLE_COLON) Emit2ArgsOP(_OP_GET);
	...
	void ClassStatement()
	{
		Lex(); 
		SQObject classname;
		SQObject ns = _fs->CreateString("", 0);
		_fs->PushTarget(0);

		while (true)
		{
			classname = Expect(TK_IDENTIFIER);
			_fs->AddInstruction(_OP_LOAD, _fs->PushTarget(), _fs->GetConstant(classname));
			if (_token != _SC('.'))
				break;
			Lex(); 
			Emit2ArgsOP(_OP_GET);

			SQInteger nl = sqi_string(ns)->_len, cl = sqi_string(classname)->_len;
			...
			{
				// ns = ns + "." + classname
				SQChar* s = _ss(_vm)->GetScratchPad(rsl(nl + cl + 2));
				memcpy(s, sqi_stringval(ns), rsl(nl));
				memcpy(s + nl, ".", rsl(1));
				memcpy(s + nl + 1, sqi_stringval(classname), rsl(cl));
				ns = _fs->CreateString(s, nl + cl + 1);
			}
		}
		...
		val = _fs->PopTarget();
		key = _fs->PopTarget();
		_fs->AddInstruction(_OP_NEWSLOTA, NEW_SLOT_STATIC_FLAG, classObj, key, val);

		// EMIT: static _namespace = <namespace>
		_fs->AddInstruction(_OP_LOAD, _fs->PushTarget(), _fs->GetConstant(_fs->_sharedstate->_namespaceidx));
		_fs->AddInstruction(_OP_LOAD, _fs->PushTarget(), _fs->GetConstant(ns));
		val = _fs->PopTarget();
		key = _fs->PopTarget();
		_fs->AddInstruction(_OP_NEWSLOTA, NEW_SLOT_STATIC_FLAG, classObj, key, val);
	...
	void EnumStatement()
	{
		Error(_SC("deprecated 'enum' keyword"));
		...
		while(_token != _SC('}')) {
			...
			sqi_table(table)->NewSlot(SQObjectPtr(key),SQObjectPtr(val));
			if(_token == ',') Lex();
		}
		SQTable *enums = sqi_table(_ss(_vm)->_consts);
		SQObjectPtr strongid = id; 
	...
	void TryExpr()
	{
		...
		_fs->AddInstruction(_OP_JMP, 0, 0);
		SQInteger jmppos = _fs->GetCurrentPos();
		_fs->SetIntructionParam(trappos, 0, _fs->PopTarget());
		_fs->SetIntructionParam(trappos, 1, (_fs->GetCurrentPos() - trappos));
	...
	void FunctionExp()
	{
		if (_token == _SC('@') || _token == TK_BY)
		{
			...
			{
				Lex();
				CreateFunction(_null_, true, true);
				_fs->AddInstruction(_OP_CLOSURE, _fs->PushTarget(), _fs->_functions.size() - 1, true);
			}
			else if (_token == _SC('{'))
			{
				CreateFunction(_null_, false, false);
				_fs->AddInstruction(_OP_CLOSURE, _fs->PushTarget(), _fs->_functions.size() - 1, true);
			}
			else
			{
		...
		else
		{
			Expect(TK_FUNCTION);
			Expect(_SC('('));
			CreateFunction(_null_, true, false);
			_fs->AddInstruction(_OP_CLOSURE, _fs->PushTarget(), _fs->_functions.size() - 1, false);
		}
	}
	void ClassExp()
	{
	...
	void CreateFunction(SQObject &name, bool hasParams = true, bool lambda = false, bool issetter = false)
	{
		SQFuncState *funcstate = _fs->PushChildState(_ss(_vm));
		funcstate->_name = name;
		SQObject paramname;
		funcstate->AddParameter(_fs->_sharedstate->_thisidx);
		funcstate->_sourcename = _sourcename;
		SQInteger defparams = 0;
		SQObject help = _null_;

		if (issetter)
		{
			funcstate->AddParameter(_fs->CreateString(_SC("value")));
		}
		...
		if (hasParams)
		{
			...
			if (_token == _SC(':'))
			{
				TypeTag(NULL); // TODO: bind to function proto
			}

			if (_token == TK_STRING_LITERAL)
			{
				if (_ss(_vm)->_enablehelp)
					help = Expect(TK_STRING_LITERAL);
				else
					Lex();
			}
		}
		...
		if(lambda) 
		{
			// @expr  or @ => expr  : a param-less lambda expr which returns the 'expr'
			// @ => { }             : a param-less lambda expr which returns a table
			// @{ stmt }            : a param-less lambda expr which executes the 'stmt'
			// @(args) => expr      : a param-ed lambda expr which return the 'expr'
			// @(args) => { }       : a param-ed lambda expr which returns a table
			// @(args) { stmt }     : a param-ed lambda expr which executes the 'stmt'
			if (_token == _SC('{'))
				Statement(false);
			else
			{
	...
	void CreateLambdaReturnExpr()
	{
		SQFuncState* funcstate = _fs->PushChildState(_ss(_vm));
		funcstate->_name = _null_;
		funcstate->AddParameter(_fs->_sharedstate->_thisidx);
		funcstate->_sourcename = _sourcename;
		
		SQFuncState* currchunk = _fs;
		_fs = funcstate;
		if (_token == TK_LAMBDA) Lex();
		Expression();
		_fs->AddInstruction(_OP_RETURN, 1, _fs->PopTarget());
		funcstate->AddLineInfos(_lex._prevtoken == _SC('\n')?_lex._lasttokenline:_lex._currentline, _lineinfo, true);
		funcstate->AddInstruction(_OP_RETURN, -1);
