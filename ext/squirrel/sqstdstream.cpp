...
static SQRegFunction _stream_methods[] = {
	_DECL_STREAM_FUNC(readblob,2,_SC("xn"),		"(size: int): blob"),
	_DECL_STREAM_FUNC(readn,2,_SC("xn"),		"(type: ['i'|'s'|'w'|'c'|'b'|'f'|'d']): int | float"),
	_DECL_STREAM_FUNC(writeblob,2,_SC("xx"),	"(blob: blob)"),
	_DECL_STREAM_FUNC(writen,3,_SC("xnn"),		"(value, type: ['i'|'s'|'w'|'c'|'b'|'f'|'d'])"),
	_DECL_STREAM_FUNC(seek,-2,_SC("xnn"),		"(pos: int, [origin='b': 'b'|'c'|'e')]"),
	_DECL_STREAM_FUNC(tell,1,_SC("x"),			"(): int"),
	_DECL_STREAM_FUNC(len,1,_SC("x"),			"(): int"),
	_DECL_STREAM_FUNC(eos,1,_SC("x"),			"(): bool"),
	_DECL_STREAM_FUNC(flush,1,_SC("x"),			"(): bool"),
...
SQRESULT declare_stream(HSQUIRRELVM v,const SQChar* name,SQUserPointer typetag,const SQChar* reg_name,SQRegFunction *methods,SQRegFunction *globals)
{
	...
	if(SQ_SUCCEEDED(sq_get(v,-3))) {
		sq_newclass(v,SQTrue,name);
		sq_settypetag(v,-1,typetag);
		sq_pushstring(v,_SC("_classname"),-1);
